...
An Anki-style review tracker for LeetCode problems, storing metadata separately from your code to enable spaced repetition. This system helps you practice problems at increasing intervals based on your recall performance, ensuring efficient long-term retention.
...
# Update the README with current problem status
python anki.py update_readme
```

### Command Details

- **add**: Creates metadata for a new problem. The optional rating parameter (1-4) lets you immediately rate your familiarity with the problem.
- **update**: Updates the review data after practicing a problem. When a rating is provided as an argument, it skips the interactive prompt.
- **reset**: Resets a problem's metadata to default values, clearing its review history. Use this if you want to start fresh with a problem.
- **list_due**: Shows all problems due for review today or that are overdue.
- **update_readme**: Regenerates this README with current problem status.

### Using the Rating Argument

The optional rating argument allows for faster workflows and scripting:

```bash
# Add a problem and immediately rate it as "Good" (3)
python anki.py add 05_1d_dp/exercises/00_climbing_stairs.py 3

# Update a problem with an "Easy" (4) rating without being prompted
python anki.py update 02_binary_search/exercises/00_binary_search.py 4
```

If you don't provide a rating, the tool will interactively prompt you.
...
This tool uses a variation of the SuperMemo 2 (SM-2) algorithm for spaced repetition:

1. **Again (Failed completely)**: Reset interval to 1 day, decrease ease factor
2. **Hard (Significant difficulty)**: Increase interval slightly, decrease ease factor
3. **Good (Some difficulty)**: Increase interval by the ease factor
4. **Easy (Perfect recall)**: Increase interval significantly, increase ease factor

The ease factor determines how quickly intervals grow between reviews. Each rating adjusts both the interval and ease factor to optimize your learning.

## Workflow Example

1. Add several problems to the system:

   ```bash
   python anki.py add 01_stack/exercises/00_valid_parentheses.py
   ```

2. Daily, check which problems are due:

   ```bash
   python anki.py list_due
   ```

3. Review each due problem, then rate your recall:

   ```bash
   python anki.py update 01_stack/exercises/00_valid_parentheses.py
   # Then enter rating 1-4 when prompted
   # Or provide rating as an argument:
   python anki.py update 01_stack/exercises/00_valid_parentheses.py 3
   ```

4. If you want to restart a problem from scratch:
   ```bash
   python anki.py reset 01_stack/exercises/00_valid_parentheses.py
   ```
