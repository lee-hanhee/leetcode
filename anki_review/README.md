...
A spaced repetition system for LeetCode problems based on the SM-2 algorithm.

## How to Use

### Getting Started

The Anki review system helps you practice LeetCode problems using spaced repetition, showing you problems at increasing intervals based on how well you remember them.
...
This adds a problem to the review system with default settings.
...
This initiates a review session for a problem. You'll be prompted to rate how well you remembered the solution:
...
Based on your rating, the system will calculate the next review date.
...
### Resetting a Problem's Review History

```bash
python anki.py reset <problem_path>
```

This resets the spaced repetition tracking for a problem, useful when you want to start fresh.

Example:

```bash
python anki.py reset 00_arrays_and_hashing/exercises/03_two_sum.py
```
...
This shows all problems that are due for review today or are overdue. Problems are sorted by their due date, with overdue problems shown first.
...
This regenerates this README file with the current status of all problems in the system.

## Review Workflow

1. Run `python anki.py list_due` to see which problems need review
2. For each due problem:
   - Open the source file to review the problem
   - Try to solve it without looking at your previous solution
   - Rate your performance using `python anki.py update <problem_path>`
3. After reviewing, the system automatically schedules the next review date
...
The ease factor determines how quickly intervals grow between reviews. Higher ease factors mean longer intervals between reviews.

## Metadata Format

Each problem's review data is stored in a JSON file in the `anki_review/metadata/` directory with the following format:
